...
namespace trackerml {
    ...
    string XMFile::getSongName() { return songName; }

    int XMFile::getRestartPosition() { return restartPosition; }

    int XMFile::getChannelsCount() { return channelsCount; }

    int XMFile::getSongLength() { return patternsTable.size(); }

    int XMFile::getTempo() { return tempo; }

    int XMFile::getBpm() { return bpm; }

    vector<int> XMFile::getPatternsTable() { return patternsTable; }

    FrequencyTableType XMFile::getFrequencyTableType() {
        return frequencyTableType;
    }

    vector<XMInstrument> XMFile::getInstruments() { return instruments; }

    vector<XMPattern> XMFile::getPatterns() { return patterns; }

    void XMFile::setSongName(string songName) { this->songName = songName; }

    void XMFile::setRestartPosition(int pos) { restartPosition = pos; }

    void XMFile::setChannelsCount(int count) { channelsCount = count; }
    ...
    void XMFile::read(istream &is) { readHeader(is); }
    ...
    void XMFile::writeHeader(ostream &os) {
        ...
        streamWriteInt(os, max(1, (const int)patternsTable.size()), 2,
                       order); // Song length
