...
namespace trackerml {
    ...
    bool StringStream::empty() { return length <= 0; }

    string StringStream::getBuffer() { return buffer; }

    unsigned int StringStream::getColumnCount() { return columnCount; }

    unsigned int StringStream::getLineCount() { return lineCount; }

    unsigned int StringStream::getLength() { return length; }

    unsigned int StringStream::getStart() { return start; }
    ...
    bool StringStream::peekIn(vector<string> compareList) {
        ...
        for (auto str : compareList) {
            if (length >= str.length() &&
                buffer.substr(start, str.length()) == str) {
                return true;
            }
        }
    ...
    string StringStream::reverseReadWhile(vector<string> compareList) {
        ...
        while (!stop) {
            if (empty()) {
                ...
                for (auto str : compareList) {
                    if (length >= str.length() &&
                        buffer.substr(start + length - str.length(),
                                      str.length()) == str) {
                        stop = false;
                        read = str + read;
                        length -= str.length();
    ...
    string StringStream::toString() { return buffer.substr(start, length); }
